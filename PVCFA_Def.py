...
DEPTH__BOUND = 18
TIME__BOUND = 9
ANALYSE__LIMIT = chess.engine.Limit(depth=DEPTH__BOUND, time=TIME__BOUND) #the first one to be saturated is the limit
MULTI__PV = 1
POINT__OF__VIEW = 1
DEPTH__PV__LIMIT = 6
...
def calculatePerspectiveValues():

    if LINEAR:
        span = PERSPECTIVE[0] - PERSPECTIVE[1]
        states_num = DEPTH__PV__LIMIT
        decrease = span / states_num

        for i in range(DEPTH__PV__LIMIT+1):
            PERSPECTIVE__VALUES.append(PERSPECTIVE[0] - decrease * i)
    else:
        for i in range(DEPTH__BOUND):
            PERSPECTIVE__VALUES.append(PERSPECTIVE[0]/(i+1))
