...
def getScoreType(pov_score):
    if SCORE__TYPE == 'cp':
        return pov_score.pov(POINT__OF__VIEW).score(mate_score=1000) / (1000)
...
class ChessPiece:
    ...
    def calculatePerturbedQValues(self, pv_board_states_list, pv_state_values_list):
        ...
        for step, (pv_board_state, pv_state_value) in enumerate(zip(pv_board_states_list, pv_state_values_list)):
            ...
            if pv_board_state.is_checkmate():
                #print("ALREADY CHECKMATE!")
                reference_analysis = self.pEngine.analyse(pv_board_state, limit=ANALYSE__LIMIT, multipv=MULTI__PV)
                self.moveListAfterPerturbation.append("checkmate")
                ...
                if removePieceAt(pv_board_state, current_square):
                    ...
                    if pv_board_state.is_checkmate():
                        ...
                        if pv_board_state.status() not in boardIrregularStatus:
                            #print(pv_board_state.status())
                            #print("YES2", pv_board_state.fen())
                            #print("PRE")
                            #print(pv_board_state.fen())
                            reference_analysis = self.pEngine.analyse(pv_board_state, limit=ANALYSE__LIMIT, multipv=MULTI__PV)
                            #print(reference_analysis[0])
                            #print("AFTER")
                            try:
                                #print("TRY")
                            ...
                            if pv_board_state.status() == chess.Status.OPPOSITE_CHECK or pv_board_state.status() == Status.IMPOSSIBLE_CHECK:
...
class ChessBoard:
    ...
    def generateBaseBoardAnalysis(self):
        ...
        self.baseReferenceAnalysis = self.cbEngine.analyse(self.board, limit=ANALYSE__LIMIT, multipv=MULTI__PV)
    ...
    def generateBoardStatesAndValuesFromMovesLine(self):
        ...
        for move in self.pvMoveList:
            ...
            reference_analysis = self.cbEngine.analyse(current_board, limit=ANALYSE__LIMIT, multipv=MULTI__PV)
...
class Analysis:
    def __init__(self, board, pieces, pv_move_list):
        ...
        self.pieces = pieces
        self.pvMoveList = pv_move_list + [" "]
        self.pvMoveStringList = [str(move) for move in pv_move_list] + [" "]
        # self.pvMoveList = [" "] + [str(move) for move in pv_move_list]
        self.pvMoveStringSequenceList = ".".join([str(move) for move in pv_move_list])
    ...
    def plotAllGraphsFAN2(self, input1, input2, input3, input4, puzzle_n):
        ...
        context_length = len(self.pvMoveStringList)
        ...
        for idx, (ax, values1, values2, values3, values4, piece) in enumerate(
            ...
            # ZERO LINE REFERENCE
            ax.hlines(y=0, linewidth=1, xmin=0, xmax=len(self.pvMoveStringList) - 1, linestyle='dotted', color='black')
            ...
            # PLOT VALUE2
            ax.plot(self.pvMoveStringList, values2[:context_length], color='goldenrod', linestyle='--', linewidth=2, label='Specificity')

            # PLOT VALUE3
            ax.plot(self.pvMoveStringList, values3[:context_length], color='salmon', linestyle='--', linewidth=2, label='Relevance')

            # PLOT VALUE4
            ax.plot(self.pvMoveStringList, values4[:context_length], color='blue', linestyle='--', linewidth=1, label='Perspective')

            # PLOT VALUE1
            ax.plot(self.pvMoveStringList, values1[:context_length], color='teal', linestyle='-', marker='o', linewidth=4,
                    markeredgecolor='white', markerfacecolor='teal', markersize=11,
                    markeredgewidth=2, label="Saliency")

            for idy, value in zip(self.pvMoveStringList, values1):
                ax.annotate(
    ...
    def representPerturbationValues(self, puzzle_n):
        ...
        # print("11111")
        ### ARROW
        first_move_arrow = [chess.svg.Arrow(self.pvMoveList[0].from_square, self.pvMoveList[0].to_square)]

        svg_board = chess.svg.board(self.board, arrows = first_move_arrow)
        ...
        dir_string = str(puzzle_n) + "." + str(returnSimplifiedFEN(self.board.fen())) + self.pvMoveStringSequenceList + ".png"
